...
The scripts in this directory are used to manage the WME (WIS2 Monitoring Events)
...
## Mapping from WME codelists to the WMO Code Registry

The overall setup of WME codelists publication to the WMO Codes Registry works as follows:
...
- each WME codelist CSV file is a `reg:Register` itself, attached to the `wis` register as a sub-register
- each row in a WME codelist CSV file is a `skos:Concept` tied to its sub-register

## Publication workflows

Managing WME codelists publication to the WMO Codes Registry involves the following steps:
